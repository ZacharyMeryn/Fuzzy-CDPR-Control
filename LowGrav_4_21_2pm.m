...
% =========================================================================
% BLOCK 1 (lines ~28-87): PARAMETERS
% everything the rest of the file depends on lives here. gravity values,
% robot geometry, cable specs, timing, disturbance setup. nothing is
% computed here — it's all definitions. fill in the placeholders before
% running on real hardware.
% =========================================================================

% gravity for each body (m/s^2) — this is the entire mechanism for
% simulating different planets. swap the number, rest handles itself
GRAVITY.earth  = 9.81;
GRAVITY.moon   = 1.62;   % primary target — 1/6 of earth
GRAVITY.mars   = 3.72;   % 0.38 of earth — second most relevant
GRAVITY.ceres  = 0.27;   % stress test — almost nothing, max sag scenario
GRAVITY.europa = 1.32;   % 0.13 of earth — ice moon wildcard
...
% workspace boundaries — EE stays 5cm from edges to avoid singularities
% (when EE is at a corner the cables go nearly parallel and lose control authority)
P.ws_min = [0.05, 0.05];
P.ws_max = [0.95, 0.95];

% EE mass — placeholder, weigh the actual printed assembly before running
P.mass_EE    = 0.5;    % kg
P.inertia_EE = 0.001;  % kg*m^2, not used in 2D translation sim but keeping it here

% the filament feed tube runs from A1 to the EE and pulls it toward A1
% 0.3 N/m means at 0.5m away it applies 0.15N toward A1 — measure this when assembled
P.tube_force_per_meter = 0.3;

% cable properties — all placeholders, get actual values from dyneema spec sheet
P.cable_diameter   = 0.001;   % m — measure with calipers
...
P.cable_stiffness  = 50000;   % N/m, axial stiffness EA/L — get from spec sheet
P.cable_damping    = 5.0;     % N*s/m, energy dissipation in cable

% how fast the cable actually "feels" a tension change from the motor
% 20 Hz is an educated guess for dyneema — needs prof check before using this number
% think of it like: motor commands new length, cable takes ~1/20th of a second to comply
P.cable_compliance_bandwidth = 20.0;   % Hz

% tension limits — T_min keeps cables from going slack (preload)
% T_max is set by motor rated torque and cable break strength
% both are placeholders — derive from hardware specs before running
P.T_min = 2.0;    % N
P.T_max = 50.0;   % N
...
% =========================================================================
% ANIMATION: IT2 vs PID side by side
% paste this at the very end of LowGrav_commented.m, after the export call
%
% what it does:
%   left panel  — IT2-FLS controlling Moon run
%   right panel — PID controlling Moon run (same trajectory, same disturbance)
%   cables change color based on tension (blue=low, red=high)
%   EE leaves a trail so you can see the path history
%   disturbance window highlighted so viewers can see exactly when the shove hits
%   exports as cdpr_moon_comparison.mp4 — drop straight into slides
%
% change body_anim below to 'Mars', 'Ceres', or 'Europa' for other bodies
% =========================================================================

body_anim = 'Moon';   % which body to animate — change this to any of the four

% pull the two logs we want to animate
log_it2 = results.(sprintf('%s_IT2', body_anim)).disturbed;   % disturbed run so the shove shows up
log_pid  = results.(sprintf('%s_PID', body_anim)).disturbed;

% tension limits for color scaling — same as simulation
T_lo = P.T_min;   % tension that maps to blue (low)
T_hi = P.T_max;   % tension that maps to red (high)

% how many timesteps to skip between frames — 1 = every timestep (slow, smooth)
% 5 = every 5th timestep (faster, still readable at 100Hz source)
% at 100Hz source and skip=5 you get 20Hz animation — perfect for video
frame_skip = 5;

% how fast the video plays back — 20 frames per second looks natural
fps = 20;

% color map for cable tension: blue (low) -> cyan -> yellow -> red (high)
% 64 steps, each cable gets interpolated to its tension level
n_colors = 64;
tension_cmap = [linspace(0,1,n_colors)', linspace(0,1,n_colors)', linspace(1,0,n_colors)'];

% ---- set up the figure ----
fig_anim = figure('Color','black','Position',[50 50 1400 700]);
fig_anim.Name = sprintf('CDPR Animation: %s — IT2 vs PID', body_anim);

% left panel: IT2
ax_it2 = subplot(1,2,1);
set(ax_it2,'Color','k','XColor','w','YColor','w','GridColor',[0.3 0.3 0.3], ...
           'FontName','Times New Roman','FontSize',10);
axis(ax_it2, 'equal');
xlim(ax_it2, [-0.15 1.15]);
ylim(ax_it2, [-0.15 1.15]);
grid(ax_it2, 'on');
hold(ax_it2, 'on');
title(ax_it2, 'IT2-FLS', 'Color','w', 'FontSize',13, 'FontWeight','bold', ...
      'FontName','Times New Roman');
xlabel(ax_it2, 'x (m)', 'Color','w', 'FontName','Times New Roman');
ylabel(ax_it2, 'y (m)', 'Color','w', 'FontName','Times New Roman');

% right panel: PID
ax_pid = subplot(1,2,2);
set(ax_pid,'Color','k','XColor','w','YColor','w','GridColor',[0.3 0.3 0.3], ...
           'FontName','Times New Roman','FontSize',10);
axis(ax_pid, 'equal');
xlim(ax_pid, [-0.15 1.15]);
ylim(ax_pid, [-0.15 1.15]);
grid(ax_pid, 'on');
hold(ax_pid, 'on');
title(ax_pid, 'PID', 'Color','w', 'FontSize',13, 'FontWeight','bold', ...
      'FontName','Times New Roman');
xlabel(ax_pid, 'x (m)', 'Color','w', 'FontName','Times New Roman');
ylabel(ax_pid, 'y (m)', 'Color','w', 'FontName','Times New Roman');

% anchor points — big white squares, clearly visible against black background
plot(ax_it2, P.anchors(:,1), P.anchors(:,2), 'ws', 'MarkerSize',12, 'MarkerFaceColor','w', 'LineWidth',1.5);
plot(ax_pid,  P.anchors(:,1), P.anchors(:,2), 'ws', 'MarkerSize',12, 'MarkerFaceColor','w', 'LineWidth',1.5);

% anchor labels
anchor_labels = {'A1','A2','A3','A4'};
offsets = [-0.08, 0.04; 0.03, 0.04; 0.03, -0.06; -0.08, -0.06];
for i = 1:4
    text(ax_it2, P.anchors(i,1)+offsets(i,1), P.anchors(i,2)+offsets(i,2), ...
         anchor_labels{i}, 'Color','w', 'FontSize',8, 'FontName','Times New Roman');
    text(ax_pid, P.anchors(i,1)+offsets(i,1), P.anchors(i,2)+offsets(i,2), ...
         anchor_labels{i}, 'Color','w', 'FontSize',8, 'FontName','Times New Roman');
end

% reference trajectory — faint dotted line
% Color(4) sets alpha transparency — MATLAB doesn't accept 'Alpha' as a plot property
rl_it2 = plot(ax_it2, log_it2.pos_ref(:,1), log_it2.pos_ref(:,2), 'w:', 'LineWidth',0.8);
rl_it2.Color(4) = 0.4;
rl_pid = plot(ax_pid, log_pid.pos_ref(:,1), log_pid.pos_ref(:,2), 'w:', 'LineWidth',0.8);
rl_pid.Color(4) = 0.4;

% initialize cable line objects — one per cable, color will update each frame
cables_it2 = gobjects(P.num_cables, 1);
cables_pid  = gobjects(P.num_cables, 1);
for i = 1:P.num_cables
    cables_it2(i) = plot(ax_it2, [P.anchors(i,1), 0], [P.anchors(i,2), 0], ...
                         '-', 'LineWidth', 2.5, 'Color', [0 0.5 1]);
    cables_pid(i)  = plot(ax_pid,  [P.anchors(i,1), 0], [P.anchors(i,2), 0], ...
                         '-', 'LineWidth', 2.5, 'Color', [0 0.5 1]);
end

% end effector markers
ee_it2 = plot(ax_it2, 0, 0, 'o', 'MarkerSize',14, 'MarkerFaceColor',[0.2 0.8 0.2], ...
              'MarkerEdgeColor','w', 'LineWidth',2);
ee_pid  = plot(ax_pid,  0, 0, 'o', 'MarkerSize',14, 'MarkerFaceColor',[1 0.4 0.1], ...
              'MarkerEdgeColor','w', 'LineWidth',2);

% trail lines — show path history behind the EE
trail_it2 = plot(ax_it2, NaN, NaN, '-', 'Color',[0.2 0.8 0.2 0.6], 'LineWidth',1.2);
trail_pid  = plot(ax_pid,  NaN, NaN, '-', 'Color',[1 0.4 0.1 0.6], 'LineWidth',1.2);
trail_x_it2 = []; trail_y_it2 = [];
trail_x_pid  = []; trail_y_pid  = [];

% disturbance indicator — flashes red during the shove window
dist_text_it2 = text(ax_it2, 0.5, -0.10, '', 'Color','r', 'FontSize',10, ...
                     'HorizontalAlignment','center', 'FontWeight','bold', ...
                     'FontName','Times New Roman');
dist_text_pid  = text(ax_pid,  0.5, -0.10, '', 'Color','r', 'FontSize',10, ...
                     'HorizontalAlignment','center', 'FontWeight','bold', ...
                     'FontName','Times New Roman');

% time display and live error readouts
time_text = annotation(fig_anim, 'textbox', [0.45 0.01 0.1 0.04], ...
    'String', 't = 0.00s', 'Color','w', 'FontSize',10, 'EdgeColor','none', ...
    'HorizontalAlignment','center', 'FontName','Times New Roman');
err_text_it2 = text(ax_it2, 0.02, 1.08, 'err: 0.0mm', 'Color',[0.2 0.8 0.2], ...
                    'FontSize',9, 'FontName','Times New Roman');
err_text_pid  = text(ax_pid,  0.02, 1.08, 'err: 0.0mm', 'Color',[1 0.4 0.1], ...
                    'FontSize',9, 'FontName','Times New Roman');

% tension colorbar on the right panel
colormap(fig_anim, tension_cmap);
cb = colorbar(ax_pid, 'eastoutside');
cb.Color = 'w';
cb.Label.String = 'Cable tension (N)';
cb.Label.Color  = 'w';
cb.Label.FontName = 'Times New Roman';
caxis(ax_pid, [T_lo T_hi]);

% ---- video writer ----
v = VideoWriter('cdpr_moon_comparison.mp4', 'MPEG-4');
v.FrameRate = fps;
v.Quality   = 95;
open(v);

fprintf('\nrendering animation (%d frames)...\n', floor(P.N / frame_skip));

% ---- main animation loop ----
for k = 1:frame_skip:P.N
    t = P.t_vec(k);
    pos_it2 = log_it2.pos_est(k,:);
    pos_pid  = log_pid.pos_est(k,:);
    T_it2 = log_it2.T_cables(k,:);
    T_pid  = log_pid.T_cables(k,:);

    % update cables — color by tension
    for i = 1:P.num_cables
        t_norm_it2 = max(0, min(1, (T_it2(i) - T_lo) / (T_hi - T_lo)));
        t_norm_pid  = max(0, min(1, (T_pid(i)  - T_lo) / (T_hi - T_lo)));
        c_idx_it2 = max(1, round(t_norm_it2 * (n_colors-1)) + 1);
        c_idx_pid  = max(1, round(t_norm_pid  * (n_colors-1)) + 1);
        set(cables_it2(i), 'XData', [P.anchors(i,1), pos_it2(1)], ...
                           'YData', [P.anchors(i,2), pos_it2(2)], ...
                           'Color', tension_cmap(c_idx_it2,:));
        set(cables_pid(i),  'XData', [P.anchors(i,1), pos_pid(1)], ...
                           'YData', [P.anchors(i,2), pos_pid(2)], ...
                           'Color', tension_cmap(c_idx_pid,:));
    end

    set(ee_it2, 'XData', pos_it2(1), 'YData', pos_it2(2));
    set(ee_pid,  'XData', pos_pid(1),  'YData', pos_pid(2));

    trail_x_it2(end+1) = pos_it2(1); %#ok<SAGROW>
    trail_y_it2(end+1) = pos_it2(2);
    trail_x_pid(end+1)  = pos_pid(1);
    trail_y_pid(end+1)  = pos_pid(2);
    set(trail_it2, 'XData', trail_x_it2, 'YData', trail_y_it2);
    set(trail_pid,  'XData', trail_x_pid,  'YData', trail_y_pid);

    in_disturbance = (t >= P.disturbance_time) && ...
                     (t <= P.disturbance_time + P.disturbance_duration);
    if in_disturbance
        set(dist_text_it2, 'String', '! DISTURBANCE !');
        set(dist_text_pid,  'String', '! DISTURBANCE !');
    else
        set(dist_text_it2, 'String', '');
        set(dist_text_pid,  'String', '');
    end

    set(err_text_it2, 'String', sprintf('err: %.1fmm', norm(log_it2.error(k,:))*1000));
    set(err_text_pid,  'String', sprintf('err: %.1fmm', norm(log_pid.error(k,:))*1000));
    set(time_text, 'String', sprintf('t = %.2fs', t));
...
close(v);
fprintf('saved: cdpr_moon_comparison.mp4\n');
fprintf('drop it straight into slides — plays on click.\n');